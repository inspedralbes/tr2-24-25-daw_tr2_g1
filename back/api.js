...
const createPIRecord = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(400)
      .json({ error: "Format de dades de l'alumne incorrecte" });
  }
...
const createAlumneEndpoint = async (req, res) => {
  try {
    const { nom, cognom, ralc, dni, data_naixement, centre_procedencia_id } =
      req.body;
    ...
    if (!nom || !cognom || !ralc || !dni) {
      return res
        ...
        .json({
          success: false,
          error: "Falten camps obligatoris (nom, cognom, ralc, dni)",
        });
    }

    const newId = await models.createAlumne({
      nom,
      cognom,
      ralc,
      dni,
      data_naixement,
      centre_procedencia_id,
    });
    res.json({
      success: true,
    ...
    if (error.code === "ER_DUP_ENTRY") {
      return res
        ...
        .json({
          success: false,
          error: "Ja existeix un alumne amb aquest DNI o RALC",
        });
    }
